...
class SmsGui(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        self.param_fields: dict[str, tk.StringVar] = {
            "n_fft": tk.StringVar(value=str(defaults.n_fft)),
            "hop_size": tk.StringVar(value=str(defaults.hop_size)),
            "peak_threshold_db": tk.StringVar(value=str(defaults.peak_detection_threshold_db)),
            "max_freq_dev_hz": tk.StringVar(
                value=str(defaults.max_partial_frequency_deviation_hz)
            ),
            "max_missed_frames": tk.StringVar(value=str(defaults.max_consecutive_missed_frames)),
            "envelope_bw_hz": tk.StringVar(value=str(defaults.envelope_section_bandwidth_hz)),
            "random_seed": tk.StringVar(value=str(defaults.random_seed)),
        }

        self._add_field(params_frame, "FFT size (N)", "n_fft")
        self._add_field(params_frame, "Hop size (H)", "hop_size")
        self._add_field(params_frame, "Peak threshold (dB)", "peak_threshold_db")
        self._add_field(params_frame, "Freq. deviation (Hz)", "max_freq_dev_hz")
        self._add_field(params_frame, "Missed frames", "max_missed_frames")
        self._add_field(params_frame, "Envelope BW (Hz)", "envelope_bw_hz")
        self._add_field(params_frame, "Random seed", "random_seed")
    ...
    def _add_field(self, parent: ttk.LabelFrame, label: str, key: str) -> None:
        row = ttk.Frame(parent)
        row.pack(fill="x", padx=8, pady=3)
        ttk.Label(row, text=label, width=22).pack(side="left")
        ttk.Entry(row, textvariable=self.param_fields[key], width=12).pack(side="left")
    ...
    def _reset_defaults(self) -> None:
        ...
        self.param_fields["n_fft"].set(str(d.n_fft))
        self.param_fields["hop_size"].set(str(d.hop_size))
        self.param_fields["peak_threshold_db"].set(str(d.peak_detection_threshold_db))
        self.param_fields["max_freq_dev_hz"].set(str(d.max_partial_frequency_deviation_hz))
        self.param_fields["max_missed_frames"].set(str(d.max_consecutive_missed_frames))
        self.param_fields["envelope_bw_hz"].set(str(d.envelope_section_bandwidth_hz))
        self.param_fields["random_seed"].set(str(d.random_seed))
        self._log("Parameters reset to paper defaults.")

    def _parse_int(self, key: str, label: str) -> int:
        raw = self.param_fields[key].get().strip()
        try:
            return int(raw)
    ...
    def _params_from_gui(self):
        return replace(
            ...
            max_partial_frequency_deviation_hz=self._parse_float(
                "max_freq_dev_hz", "Freq. deviation"
            ),
